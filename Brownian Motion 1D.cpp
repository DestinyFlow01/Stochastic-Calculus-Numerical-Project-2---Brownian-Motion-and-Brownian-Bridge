...
class Particle {
	...
	public : 
		void Initialization();
		void Brownian_Motion(int N, double dt); // Generating Brownian motion from current time step to next time step
...
void Particle::Brownian_Motion(int N, double dt) {
	...
	for(int j = 1; j<N; j++) {
		//Sampling only 1 random number : 
		double dx_current = d(gen);
		dx.push_back(dx_current);
		
//		cout<<"dx = "<<dx_current<<endl;
		
		// Ensure the vector `x` is not empty
	    if (x.empty()) {
	        throw runtime_error("Vector x is empty. Ensure Initialization() is called before Brownian_Motion().");
	    }
...
# Read the CSV file
data_particle = pd.read_csv('1D trajectory of particles (Brownian Motion).csv')
data_mean = pd.read_csv('1D trajectory of particles (Brownian Motion) Mean.csv')
data_variance = pd.read_csv('1D trajectory of particles (Brownian Motion) Variance.csv')
data_covariance = pd.read_csv('1D trajectory of particles (Brownian Motion) Covariance.csv')
data_correlation = pd.read_csv('1D trajectory of particles (Brownian Motion) Correlation.csv')
...
plt.xlabel('Time')
plt.ylabel('Position')
plt.title('Brownian Motion 1D Trajectories')
plt.grid()
...
plt.xlabel('Time')
plt.ylabel('Mean')
plt.title('Brownian Motion 1D Trajectories (Mean)')
plt.grid()
plt.show(block=False)  

plt.figure(3)
plt.plot(time_variance, data_variance['variance'])
plt.plot(time_variance, time_variance, 'k--', linewidth=2,)
plt.xlabel('Time')
plt.ylabel('Variance')
plt.title('Brownian Motion 1D Trajectories (Variance)')
plt.grid()
plt.show(block=False)  
...
plt.title("Heatmap of Covariance Matrix for Brownian Motion")
plt.xlabel("Time")
plt.ylabel("Time")
plt.tight_layout()
plt.show(block=False)
...
plt.colorbar(label='Value')
plt.title("Heatmap of Correlation Matrix for Brownian Motion")
plt.xlabel("Time")
plt.ylabel("Time")
plt.tight_layout()
plt.show(block=False)
...
# Analytical covariance solution for Brownian motion
def analytical_covariance(t, n):
    T = np.array(t)
    return np.minimum.outer(T, T)  # Covariance for Brownian motion

# Analytical correlation matrix (assumes variance grows linearly with time)
def analytical_correlation(t, n):
    analytical_cov = analytical_covariance(t, n)
...
numerical_cov = data_covariance.iloc[time_index, 1:].values  # Proper slicing for pandas DataFrame
numerical_corr = data_correlation.iloc[time_index, 1:].values
analytical_cov = analytical_covariance(time_covariance, len(time_covariance))[time_index, :]
analytical_corr = analytical_correlation(time_covariance, len(time_covariance))[time_index, :]

# Plot initial Covariance
num_cov_plot, = ax1.plot(time_covariance, numerical_cov, label="Numerical Covariance")
ana_cov_plot, = ax1.plot(time_covariance, analytical_cov, 'k--', linewidth=2, label="Analytical Covariance")
ax1.set_xlabel('Time')
ax1.set_ylabel('Covariance')
ax1.set_title(f'Covariance Comparison at Time t={time_covariance[time_index]:.2f}')
ax1.legend()
ax1.grid()

# Plot initial Correlation
num_corr_plot, = ax2.plot(time_correlation, numerical_corr, label="Numerical Correlation")
ana_corr_plot, = ax2.plot(time_correlation, analytical_corr, 'k--', linewidth=2, label="Analytical Correlation")
ax2.set_xlabel('Time')
ax2.set_ylabel('Correlation Coefficient')
ax2.set_title(f'Correlation Comparison at Time t={time_correlation[time_index]:.2f}')
ax2.legend()
ax2.grid()

# Slider for updating plots
time_slider = Slider(slider_ax, 'Time Index', 0, len(time_covariance) - 1, valinit=time_index, valstep=1)
...
def update(val):
    ...
    numerical_cov = data_covariance.iloc[time_index, 1:].values  # Corrected slicing
    numerical_corr = data_correlation.iloc[time_index, 1:].values
    analytical_cov = analytical_covariance(time_covariance, len(time_covariance))[time_index, :]
    analytical_corr = analytical_correlation(time_covariance, len(time_covariance))[time_index, :]
    ...
    ana_cov_plot.set_ydata(analytical_cov)
    ax1.set_title(f'Covariance Comparison at Time t={time_covariance[time_index]:.2f}')
    ax1.relim()
    ax1.autoscale_view()
    ...
    ana_corr_plot.set_ydata(analytical_corr)
    ax2.set_title(f'Correlation Comparison at Time t={time_correlation[time_index]:.2f}')
    ax2.relim()
    ax2.autoscale_view()
...
int main() {	
    ...
    for(int i = 0; i<num_particle; i++) {
    	if(num_particle > 1000) {
    		if(i%t_out == 0) {
		    	...
		    	//Brownian motion
		    	cout<<"Simulating : ";
		    	particles[i].Brownian_Motion(N, dt);
				cout<<"Done\n\n";
			}
			else {  	
		    	//Initialization
	...
	cout<<"- Trajectories : ";
	string name = "1D trajectory of particles (Brownian Motion).csv"; OutputCSV_1D(name, dt, particles, N);   
	cout<<"Done\n"; 	
	
	cout<<"- Mean : ";
	name = "1D trajectory of particles (Brownian Motion) Mean.csv"; OutputCSV_StatQuan1D(name, dt, mean, N, "mean");
	cout<<"Done\n";
	
	cout<<"- Variance : ";
	name = "1D trajectory of particles (Brownian Motion) Variance.csv"; OutputCSV_StatQuan1D(name, dt, variance, N, "variance");
	cout<<"Done\n";
	
	cout<<"- Covariance : ";
	name = "1D trajectory of particles (Brownian Motion) Covariance.csv"; OutputCSV_StatQuan2D(name, dt, covariance, N, "Cov");
	cout<<"Done\n";
	
	cout<<"- Correlation : ";
	name = "1D trajectory of particles (Brownian Motion) Correlation.csv"; OutputCSV_StatQuan2D(name, dt, correlation, N, "Corr");
    ...
    /*
    Type this in CMD
    g++ "Brownian Motion 1D.cpp" -IC:\Python313\include -LC:\Python313\libs -lpython313 -o "Brownian Motion 1D" -Wl,--enable-auto-import
	"Brownian Motion 1D.exe"
    */
}