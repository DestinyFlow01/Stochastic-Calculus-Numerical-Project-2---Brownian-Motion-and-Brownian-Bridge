...
class Particle {
	...
	public : 
		void Initialization();
		void Brownian_Bridge(int N, double dt); // Generating Brownian motion from current time step to next time step
...
void Particle::Brownian_Bridge(int N, double dt) {
...
plt.plot(time_variance, time_variance*(time_variance.iloc[-1]-time_variance), 'k--', linewidth=2,)
plt.xlabel('Time')
plt.ylabel('Variance')
...
def analytical_covariance(s, t, T=1.0):
    """
    Computes the analytical covariance for a Brownian Bridge: 
    Cov(Z_t, Z_s) = min(s, t) - (s * t) / T.

    Parameters:
    s (float or np.ndarray): Time points s.
    t (float or np.ndarray): Time points t.
    T (float): Total time duration, default is 1.
    ...
    covariance = np.minimum.outer(s, t) - (np.outer(s, t) / T)
    return covariance

# Analytical correlation matrix for Brownian Bridge
def analytical_correlation(s, t, T=1.0):
    """
    Computes the analytical correlation matrix for a Brownian Bridge.
    """
...
int main() {	
    ...
    for(int i = 0; i<num_particle; i++) {
    	if(num_particle > 100) {
    		if(i%t_out == 0) {
		    	...
		    	//Brownian motion
		    	cout<<"Simulating : ";
		    	particles[i].Brownian_Bridge(N, dt);
				cout<<"Done\n\n";
			}
			else {  	
		    	//Initialization
    ...
    /*
    Type this in CMD
    g++ "Brownian Bridge 1D.cpp" -IC:\Python313\include -LC:\Python313\libs -lpython313 -o "Brownian Bridge 1D" -Wl,--enable-auto-import
	"Brownian Bridge 1D.exe"
    */
}